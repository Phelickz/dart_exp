...
class JwtExpress {
  ...
  /// See the [JwtExpress] constructor for details of how [defaultIatExp]
  /// and [maxAge] control these default values.
  ///
  /// Throws [JwtExpressException.invalidToken] if the Map is not suitable.
  factory JwtExpress.fromMap(Map<dynamic, dynamic> data,
    ...
    for (var claimName in ['iss', 'sub', 'jti']) {
      if (data.containsKey(claimName)) {
        ...
        if (v is String) {
          ...
          throw JwtExpressException.invalidToken; // claim is not a StringOrURI
        }
      }
    }
    ...
    if (data.containsKey('aud')) {
      ...
      if (aud is String) {
        ...
        throw JwtExpressException.invalidToken; // unexpected type for audience
      }
    }
    ...
    data.forEach((k, v) {
      if (k is String) {
        ...
        throw JwtExpressException.invalidToken; // Map had non-String as a key
      }
    });

    // Create a new JwtExpress and initialize with the registered claims
    return JwtExpress(
  ...
  void validate(
    ...
    if (issuer != null) {
      if (issuer != this.issuer) {
        throw JwtExpressException.incorrectIssuer;
      }
    }
    ...
    if (audience != null) {
      if (this.audience != null && !this.audience!.contains(audience)) {
        throw JwtExpressException.audienceNotAllowed;
      }
    }
    ...
    if (expiry != null && !cTime.isBefore(expiry!.add(absClockSkew))) {
      throw JwtExpressException.tokenExpired;
    }
    ...
    if (notBefore != null && notBefore!.subtract(absClockSkew).isAfter(cTime)) {
      throw JwtExpressException.tokenNotYetAccepted;
    }
